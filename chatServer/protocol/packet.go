...
func protocolInitHeaderSize() int16 {
	...
	headerSize := gohipernetFake.Sizeof(reflect.TypeOf(packetHeader))
...
// decode the header by reader 2 * 8 byte, 2 * 8 byte, 1 byte
func DecodingPacketHeader(header *Header, data []byte) {
	reader := gohipernetFake.MakeReader(data, true)
	header.TotalSize, _ = reader.ReadS16() // read 2 * 8 byte and return it to first, second is error. signed value
	header.ID, _ = reader.ReadS16()
	header.PacketType, _ = reader.ReadS8()
}

// encode packet header a size, packetid, packettype
func EncodingPacketHeader(writer *gohipernetFake.RawPacketData, totalSize int16, pktId int16, packetType int8) {
...
func (loginReq LoginReqPacket) EncodingPacket() ([]byte, int16) {
	...
	writer := gohipernetFake.MakeWriter(sendBuf, true) // writer is of RawPacketData type
...
func (loginReq *LoginReqPacket) Decoding(bodyData []byte) bool {
	...
	reader := gohipernetFake.MakeReader(bodyData, true)
...
func (loginRes LoginResPacket) EncodingPacket() ([]byte, int16) {
	...
	writer := gohipernetFake.MakeWriter(sendBuf, true)
...
func (response ErrorNtfPacket) EncodingPacket(errorCode int16) ([]byte, int16) {
	...
	writer.WriteS16(errorCode)//writer.WriteS16(response.ErrorCode)
	return sendBuf, totalSize
}
...
/// 방에 있는 유저 리스트 통보(채널에 들어오는 유저들에게)
